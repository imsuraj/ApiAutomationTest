...
const generateUUID = () => {
    ...
    const generateRandomPart = () => Cypress._.random(0, 0xffff).toString(16).padStart(4, '0')
  
    // Create the UUID format
    const uuid = `${generateRandomPart()}${generateRandomPart()}-${generateRandomPart()}-4${generateRandomPart().substring(1)}-a${generateRandomPart().substring(1)}-${generateRandomPart()}${generateRandomPart()}${generateRandomPart()}`
  
    return uuid
  }


const getRequestBodyForRetrivePaymentOrder = () => {
    return {
...
module.exports = {
    ...
    generateUUID,
    getRequestBodyForRetrivePaymentOrder,
    successRedirectUrl,
    failureRedirectUrl,
    currencies,
    maxAmountAED,
